...
class TemplatesSingleton:
    ...
    def load_template_names(self):
        return [x for x in os.listdir(os.path.join(self.TEMPLATE_DIR, 'tasks')) if x.endswith('.json')]

    def load_task(self, file_name: str):
        if file_name not in self._templates:
            file_name = random.choice(self._templates)
        with open(os.path.join(self.TEMPLATE_DIR, 'tasks', file_name), 'r', encoding='utf-8') as f:
            return json.loads(f.read())

    def load_response(self, file_name: str):
        if file_name not in self._templates:
            file_name = random.choice(self._templates)
        with open(os.path.join(self.TEMPLATE_DIR, 'responses', file_name), 'r', encoding='utf-8') as f:
            return json.loads(f.read())
    ...
    def load_cache(self):
        ...
        self._cache = {self.make_hash(self.load_task(x)['contractCode']): self.load_response(x) for x in templates}
...
async def contract_report(request: Request):
    ...
    response = [
        VulnerabilityReport(**x)
        for x in dummy_contracts.get_response(solidity)
    ]
